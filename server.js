...
let connection;
let sessionStore; // Declare sessionStore here
...
sshConnection.on('ready', () => {
  ...
  sshConnection.forwardOut(
    ...
    (err, stream) => {
      ...
      createDatabaseConnection(stream);
      ...
      // Use session middleware after sessionStore is initialized
      app.use(session({
...
function createDatabaseConnection(stream) {
...
function reconnect() {
  ...
  setTimeout(() => {
    ...
    sshConnection.forwardOut(
      ...
      (err, stream) => {
        if (err) {
          console.error('Port forwarding error during reconnection:', err);
          return;
        }
        console.log('Port forwarding re-established');
        createDatabaseConnection(stream);
      }
    );
...
function startServer() {
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      console.error('Port 5000 is already in use. Retrying...');
      setTimeout(() => {
        server.close();
        startServer();
      }, 1000);
...
// Call startServer() after establishing the SSH connection
